...
- [x] BUG-013 Page-thumbnail hover icons overlapped — the drag-handle hint (top-left) and the duplicate/delete buttons (top-right) collided on a portrait page, whose thumbnail is only ~40px wide at the strip's 52px height. The two buttons now sit together in one pill tucked into the top-right corner and the decorative drag hint moved to the bottom-left, so they stay clear at any page aspect

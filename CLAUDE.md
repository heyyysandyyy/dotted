...
- [ ] PHOTO-011 Selection and local masking (large, foundational — marquee/lasso/magic wand selection, layer masks, graduated/radial local adjustment brush; unlocks locally-scoped versions of PHOTO-007/008/010 tools, so worth sequencing before those where possible). Phase 1 shipped: rectangle/ellipse marquee, lasso and magic wand selections (new/add/subtract, feather, invert, deselect) that confine every tonal/colour/detail adjustment to the selection. Phase 2 shipped: adjustment layers — each with its own full tone/colour/detail settings, its own mask (a selection drawn while the layer is the active target; no mask = whole photo), opacity and visibility, applied in stack order; add/rename/reorder/delete, undoable. Still to come: the local adjustment brush and graduated/radial filters (as new mask kinds on the same layers)

...
/**
 * The product template's guide layer (PROD-001), pinned above the real layers.
 *
 * It looks like a layer and toggles like one, but it deliberately isn't a
 * canvas object: the guides are drawn on an overlay outside the fabric canvas
 * (ProductGuides.tsx), which is what keeps them out of every export. So this
 * row has no drag handle and no rename — there is nothing in the object tree
 * to reorder or select — and its lock icon is a statement rather than a
 * control: guide circles can never be selected or moved.
 */
function GuideLayerRow({ spec }: { spec: ProductGuideSpec }) {
  const visible = useCanvasStore((s) => s.showProductGuides)
  const toggle = useCanvasStore((s) => s.toggleProductGuides)

  return (
    <div
      className={`flex items-center gap-1 rounded px-1 py-1 text-xs text-editor-text-secondary ${
        visible ? '' : 'opacity-40'
      }`}
    >
      <span className="inline-block w-3.5 shrink-0" />
      ...
      <button
        onClick={toggle}
        title={visible ? 'Hide template guides' : 'Show template guides'}
        className="shrink-0 text-editor-text-muted hover:text-editor-text"
      >
...
export function LayersPanel() {
  ...
  const productSpec = pages.find((p) => p.id === activePageId)?.product
    ...
    <div className="flex h-full flex-col">
      ...
      {open && (
        <div className="flex-1 overflow-y-auto px-2 pb-2">
          {productSpec && <GuideLayerRow spec={productSpec} />}
          {rows.length === 0 ? (

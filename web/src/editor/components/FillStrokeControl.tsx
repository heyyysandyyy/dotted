    ...
    <div className="relative flex items-center justify-between text-xs text-neutral-400">
      ...
      <div className="relative h-9 w-10">
        ...
        <button
          ...
          className={`absolute left-[14px] top-[10px] h-6 w-6 overflow-hidden rounded-sm border shadow-sm ${
            active === 'stroke' ? 'border-indigo-400' : 'border-neutral-500'
          }`}
          style={{ ...swatchStyle(stroke), zIndex: active === 'stroke' ? 30 : 10 }}
        >
          <span className="absolute inset-[5px] rounded-[1px] bg-neutral-900" />
        </button>
        {allowFill && (
          <button
            type="button"
            ...
            className={`absolute left-0 top-0 h-6 w-6 rounded-sm border shadow-sm ${
              active === 'fill' ? 'border-indigo-400' : 'border-neutral-500'
            }`}
      ...
      {open && (
        <>
          ...
          <div
            className="absolute right-0 top-9 z-20 w-56 rounded-lg border border-neutral-700 bg-neutral-900 p-3 shadow-xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="mb-2 flex items-center justify-between">
              <span className="text-xs font-medium capitalize text-neutral-200">{active}</span>
              <button
                type="button"
                onClick={() => updateActive({ [active]: '' })}
                className={`rounded px-2 py-0.5 text-[11px] ${
                  isNone(cur) ? 'bg-black text-white' : 'bg-neutral-800 text-neutral-300 hover:bg-neutral-700'
                }`}
              >
                None
              </button>
